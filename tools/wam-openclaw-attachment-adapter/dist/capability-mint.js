...
export function mintCapabilityId() {
...
export function lstatIdentity(absolutePath) {
...
export function mintPendingBinding(input, nowMs = Date.now()) {
...
function identityMatches(binding, id) {
    if (id.isSymlink)
        return { ok: false, reason: "symlink_rejected" };
    if (!id.isFile)
        return { ok: false, reason: "not_regular_file" };
    if (id.nlink > 1)
        return { ok: false, reason: "hard_link_rejected" };
    if (id.sizeBytes !== binding.sizeBytes)
    ...
    if (id.mtimeMs !== binding.mtimeMs)
        return { ok: false, reason: "mtime_mismatch" };
    return { ok: true };
}
/**
 ...
 */
export function revalidateBindingFile(binding) {
    try {
        ...
        if (!post.ok)
            return { ok: false, reason: "toctou_" + post.reason };
        if (!binding.sha256) {
