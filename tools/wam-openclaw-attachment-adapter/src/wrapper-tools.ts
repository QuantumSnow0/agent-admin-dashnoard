...
import {
  advanceLifecycle,
  assertToolAllowedAtLifecycle,
  lifecycleTargetForTool,
} from "./lifecycle.js";
import { logAttachmentLifecycle, type LifecycleLogger } from "./lifecycle-log.js";
import type { McpBridge, McpDocumentTool } from "./mcp-bridge.js";
import type { TurnIdentity, WrapperToolName, AttachmentBinding } from "./types.js";
import { RAW_DOCUMENT_MCP_PREFIX } from "./types.js";

const WRAPPER_TO_MCP: Record<WrapperToolName, McpDocumentTool> = {
...
export async function executeWrapperTool(opts: {
  ...
  if (!resolved.ok) {
    ...
    return {
      ...
      denied: true,
      userMessage:
        category === "wait_timeout" || category === "instruction_not_observed"
  ...
  if (!life.ok) {
    return {
      ...
      denied: true,
      userMessage:
        life.reason === "must_inspect_before_parse"
...
export function syncBindingForTools(
  store: CapabilityStore,
  turn: TurnIdentity,
...
export function toSafeToolPayload(
  tool: WrapperToolName,
...
function extractSafeDocumentMeta(
  binding: AttachmentBinding,
  data: unknown,
  ...
  return {
    ...
    type: ext,
    sheet_name:
      nested.sheet_name ?? nested.sheetName ?? nested.active_sheet ?? null,
    row_estimate:
      nested.row_estimate ??
      nested.rowEstimate ??
      nested.row_count ??
...
function redactCapabilityIds(data: unknown): unknown {
...
function denyMismatch(reason: string): WrapperResult {
  return {
    ok: false,
    denied: true,
    userMessage:
...
export function redactHostPaths(data: unknown): unknown {
  if (data == null) return data;
  const text = JSON.stringify(data);
  const redacted = text
...
export function shouldBlockRawDocumentTool(toolName: string): boolean {
