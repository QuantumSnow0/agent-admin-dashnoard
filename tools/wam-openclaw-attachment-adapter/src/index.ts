...
import {
  assertStdioMcpBridgeAvailable,
  createStdioMcpBridge,
  resolveStdioMcpBridgeConfigFromEnv,
  type McpBridge,
} from "./mcp-bridge.js";
...
} from "./wrapper-tools.js";
import type { TurnIdentity, WrapperToolName } from "./types.js";
...
export type PluginApiLike = {
  ...
  registerTool?: (
    tool:
      ...
      | ((ctx: Record<string, unknown>) => Record<string, unknown> | null),
    opts?: Record<string, unknown>,
  ) => void;
...
export function createAdapterRuntime(opts: {
  ...
  if (!opts.bridge || opts.bridge.kind !== "stdio") {
    throw new Error(
      "production_bridge_required: createAdapterRuntime rejects mock/missing bridges",
    );
  }
  const roots = opts.attachmentRoots ?? resolveAttachmentRootsFromEnv();
  if (!roots.length) {
    throw new Error(
      "attachment_roots_required: set WAM_ATTACHMENT_ROOTS or WAM_AI_ATTACHMENT_ROOTS",
    );
  }
  return {
...
export function turnFromCtx(
  event: Record<string, unknown>,
  ctx: Record<string, unknown>,
  agentId: string,
): TurnIdentity {
  const delivery = ctx.deliveryContext as Record<string, unknown> | undefined;
  return {
    sessionKey: String(
      ctx.sessionKey ?? event.sessionKey ?? delivery?.sessionKey ?? "",
    ),
...
export function syncInvalidateIfTurnMismatch(
...
export function registerAttachmentAdapter(
  api: PluginApiLike,
  runtime: AdapterRuntime,
  ...
  if (runtime.bridge.kind === "mock" && !runtime.allowMockBridge) {
    api.logger?.warn?.(
      "[wam-attachment-adapter] refused: mock bridge not allowed in production registration",
    );
  ...
  if (!guard.ok) {
    api.logger?.warn?.(
      `[wam-attachment-adapter] refused to register: ${guard.reason}`,
    );
  ...
  api.on?.("message_received", (event, ctx) => {
    ...
    if (result.status === "pending") {
      ...
      api.logger?.info?.(
        `[wam-attachment-adapter] pending attachment index=${result.attachmentIndex} lifecycle=pending`,
      );
      ...
      api.logger?.warn?.(
        `[wam-attachment-adapter] pending capture fail-closed: ${result.reason} missing=${(result.missingTrustedFields ?? []).join(",")}`,
      );
    }
  });
  ...
  api.on?.("command:new", (_event, ctx) => {
    ...
    if (sessionKey) runtime.store.onSessionReset(sessionKey);
  });
  ...
  api.on?.("before_tool_call", (event, ctx) => {
    ...
    if (shouldBlockRawDocumentTool(name)) {
      return {
        block: true,
        blockReason:
          "Raw wam.business.documents.* path tools are denied; use pathless inspect/parse/reconcile wrappers after the file->instruction handshake.",
      };
    }
        ...
        return {
          ...
          async execute(_id: string, params: Record<string, unknown>) {
            ...
            const result = await executeWrapperTool({
              tool: toolName as WrapperToolName,
              store: runtime.store,
  ...
  api.logger?.info?.(
    `[wam-attachment-adapter] v0.2.2 registered openclaw=${guard.version} bridge=${runtime.bridge.kind} handshake=sequence-binding wrappers=always-catalogued core_patch=not_required`,
  );
  return { ok: true };
}

function wrapperDescription(tool: WrapperToolName): string {
  switch (tool) {
...
const plugin = definePluginEntry({
  ...
  description:
    "v0.2.2 plugin-only sequence binding on stock OpenClaw 2026.7.1-2: wrappers always catalogued; execute late-claims after bounded wait for instruction_observed.",
  register(api) {
    const version = resolveOpenClawVersion({
      apiConfig: api.config as Record<string, unknown> | undefined,
    });
    ...
    try {
      ...
      api.logger?.warn?.(
        `[wam-attachment-adapter] refused: ${
    ...
    if (!available.ok) {
      api.logger?.warn?.(
        `[wam-attachment-adapter] refused: ${available.reason}`,
      );
      return;
    }
    ...
    try {
      ...
      const runtime = createAdapterRuntime({
        ...
        agentId:
          (api.config as { agentId?: string } | undefined)?.agentId ??
          "bonface-owner",
      });
      registerAttachmentAdapter(api as PluginApiLike, runtime);
    } catch (err) {
      api.logger?.warn?.(
        `[wam-attachment-adapter] refused: ${
          err instanceof Error ? err.message : String(err)
...
export {
  capturePendingAttachment,
  captureStagedAttachment,
  buildAttachmentPromptHint,
  buildFileAckPromptHint,
...
export {
  ...
  mergePluginsAllow,
  ADAPTER_PLUGIN_ID,
  REQUIRED_WRAPPER_ALSO_ALLOW,
} from "./openclaw-config-policy.js";
export {
  executeWrapperTool,
  shouldBlockRawDocumentTool,
} from "./wrapper-tools.js";
