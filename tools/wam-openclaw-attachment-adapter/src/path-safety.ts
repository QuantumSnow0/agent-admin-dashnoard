...
export function resolveAttachmentRootsFromEnv(
  env: NodeJS.ProcessEnv = process.env,
): string[] {
  return parseAttachmentRoots(
    env.WAM_ATTACHMENT_ROOTS ?? env.WAM_AI_ATTACHMENT_ROOTS ?? null,
  );
}
 ...
 */
export function assertSafeStagedAttachmentPath(
  ...
  if (!roots.length) {
    throw new AttachmentRootError(
      "roots_not_configured",
      "WAM_ATTACHMENT_ROOTS / WAM_AI_ATTACHMENT_ROOTS is not configured",
    );
  }
  ...
  if (rootIndex < 0) {
    throw new AttachmentRootError(
      "path_outside_roots",
      "Path outside allowlisted attachment roots",
    );
  }
  ...
  if (!isPathInsideRoot(real, root)) {
    throw new AttachmentRootError(
      "realpath_outside_roots",
      "realpath escaped attachment root",
    );
  }
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    if (isLast) {
      ...
      if (typeof st.nlink === "number" && st.nlink > 1) {
        throw new AttachmentRootError(
          "hard_link_rejected",
          "Hard-linked attachment rejected",
        );
      }
      return {
        absolutePath,
