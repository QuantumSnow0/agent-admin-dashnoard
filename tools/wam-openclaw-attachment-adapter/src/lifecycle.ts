...
const ORDER: Record<CapabilityLifecycleState, number> = {
  pending: 0,
...
export function canAdvanceLifecycle(
  ...
  if (next === "consumed") return true;
  return ORDER[next] >= ORDER[current];
}

export function lifecycleTargetForTool(
 ...
 */
export function assertToolAllowedAtLifecycle(
