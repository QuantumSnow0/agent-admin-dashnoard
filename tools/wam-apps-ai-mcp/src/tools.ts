...
/** Owner/partner-only analytics tools (not available to ai_service). */
const OWNER_PARTNER_ANALYTICS_TOOLS = new Set<ToolName>([
...
export async function executeBusinessTool(opts: {
  ...
  if (
    OWNER_PARTNER_ANALYTICS_TOOLS.has(tool) &&
    !OPENBOOK_ALLOWED_ROLES.includes(actor.actorRole)
  ) {
    const audit = await db.recordAudit({
      ...auditBase(correlationId, tool, actor, opts.args),
      dataClassification: "denied",
    ...
    return {
      ...
      error: {
        category: "denied",
        message: "Agent registration performance is limited to technical_owner and business_partner",
      },
    };
  }
  ...
  try {
    ...
    if (tool === "get_agent_registration_performance") {
      validateRequiredRange(parsed.from as string, parsed.to as string);
