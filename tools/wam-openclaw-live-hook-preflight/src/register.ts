...
export function tryRegisterPreflightPlugin(
  api: PluginApiLike,
  env: NodeJS.ProcessEnv = process.env,
): { ok: boolean; reason: string } {
  if (!isPreflightExplicitlyEnabled(env)) {
    api.logger?.warn?.(
      "[wam-live-hook-preflight] refused: set WAM_LIVE_HOOK_PREFLIGHT_ENABLED=1 for explicit operator enablement",
    );
  ...
  if (!guard.ok) {
    api.logger?.warn?.(
      `[wam-live-hook-preflight] refused: ${guard.reason}`,
    );
  ...
  api.logger?.info?.(
    `[wam-live-hook-preflight] active (read-only) openclaw=${guard.version}`,
  );
